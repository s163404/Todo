...
extension TodoEntity {
    ...
    // CoreData DBのテーブルにデータを作成
    static func create(in managedObjectContext: NSManagedObjectContext, // DB操作のインスタンス
                       category: Category,
                       task: String,
                       time: Date? = Date()) {
        let todo = self.init(context: managedObjectContext)     // Todoエンティティデータを作成
        print(task)
        todo.time = time
